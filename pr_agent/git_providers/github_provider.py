...
class GithubProvider(GitProvider):
    ...
    def get_diff_files(self) -> list[FilePatchInfo]:
        ...
        try:
            ...
            # When minimize_api_calls is enabled and not in incremental mode,
            # skip file content fetches for files that already have a patch from
            # the PR files endpoint. This eliminates up to 2N API calls (N = files).
            minimize = get_settings().get("github.minimize_api_calls", False)
            skip_content = minimize and not self.incremental.is_incremental
            ...
            for file in files:
                ...
                elif skip_content and patch:
                    # Patch exists from PR files endpoint — no content fetch needed.
                    # extend_patch() is bypassed via disable_extra_lines in get_pr_diff().
                    # handle_patch_deletions() uses edit_type instead of empty content check.
                    new_file_content_str = ""
                    original_file_content_str = ""
                else:
    ...
    def get_languages(self):
        ...
        self._languages = self._get_repo().get_languages()
        return self._languages
    ...
    def get_commit_messages(self):
        ...
        try:
            commit_list = self.pr_commits if self.pr_commits is not None else list(self.pr.get_commits())
