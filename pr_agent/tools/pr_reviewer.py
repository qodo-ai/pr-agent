...
class PRReviewer:
    ...
    async def run(self) -> None:
        try:
            ...
            minimize_api_calls = get_settings().get("github.minimize_api_calls", False)
            if (
                get_settings().config.publish_output
                and not get_settings().config.get("is_auto_command", False)
                and not minimize_api_calls
            ):
    ...
    def set_review_labels(self, data):
        ...
        if (
            ...
            try:
                ...
                # Use labels from PR object (already fetched during init) to avoid
                # an extra GET call. When minimize_api_calls is enabled this
                # eliminates the GET; PUT only fires when labels actually change.
                minimize = get_settings().get("github.minimize_api_calls", False)
                if minimize:
                    current_labels = self.git_provider.get_pr_labels(update=False)
                else:
