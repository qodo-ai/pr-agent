...
!!! info "Prerequisites"
    - RAG is available only for Codium enterprise plan users, with single tenant or on-premises setup.
    - Database setup and codebase indexing must be completed before proceeding. [Contact support](https://www.codium.ai/contact/) for more information.
...
=== "`/ask`"
    The [`/ask`](../../tools/ask/) tool can access broader repository context through the RAG feature when answering questions that go beyond the PR scope alone.
...
=== "`/review`"
    The [`/review`](../../tools/review/) tool offers the _Focus area from RAG data_ which contains feedback based on the RAG references analysis.
...
- **Deployment**: Only available for PR-Agent Enterprise plan using single tenant or on-premises setup
